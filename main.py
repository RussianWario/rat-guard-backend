...
# РАЗРЕШАЕМ ПОДКЛЮЧЕНИЯ (CORS) — чтобы фронтенд мог общаться с бэкендом
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"], 
...
async def get_profile(user_id: str):
    try:
        # 1. Очищаем ID от мусора Telegram (все, что после & или %)
        clean_id = user_id.split('&')[0].split('%')[0]
        user_id_int = int(clean_id)
        
        # 2. Запрос в таблицу 'profiles' (как ты создала в SQL Editor)
        # Ищем по колонке 'id'
        response = supabase.table("profiles").select("*").eq("id", user_id_int).execute()
        
        # 3. Если пользователя нет в базе — создаем его
